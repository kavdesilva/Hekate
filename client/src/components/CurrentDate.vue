<template>
    <div>
        <div className="record-form-container">
            <h2 className="selected-date">{{currentDate.toUTCString().slice(5, 16).toLowerCase()}}</h2>
        ...
        methods: {
            ...
            deleteRecord() {
                if (confirm(`are you sure you want to delete record for ${this.currentDate.toUTCString().slice(5, 16).toLowerCase()}?`)){
                    db.collection(`${this.currentUser.username}-records`).doc({ date: `${this.currentDate.toISOString().slice(0, 10)}`}).delete()
                } else {
                    return
                }
            }
